...
interface QuotationDataResponse {
  ...
  quotationItems?: Array<{
    ...
    product?: {
      description?: string;
      cost?: number; // Needed to pre-fill cost on edit
    };
  }>;
}
...
export default function EditQuotation() {
  ...
  const calculateItemMath = useCallback((item: ExtendedQuotationItem, mode: PricingMode, gMargin: number): ExtendedQuotationItem => {
    let basePrice = 0;
    ...
    try {
      ...
      if (q) {
        ...
        if (q.quotationItems && q.quotationItems.length > 0) {
          // Initialize items in manual mode since the DB only stores final base prices
          setItems(
            q.quotationItems.map((item) => {
              const baseItem: ExtendedQuotationItem = {
                productId: item.productId,
                quantity: item.quantity,
                cost: item.product?.cost || 0, // Fallback to product cost if available via include
                margin: 0,
                taxMode: "net", // DB stores base prices, so default to net
                typedPrice: item.unitPrice, // Load unitPrice into typedPrice for manual editing
              ...
              // Run it through the calculator to populate margin context
